...
import { PROYECT, api_post, HOST_MONTHLY_PURCHASE, api_get, api_put } from '../../services/api'
...
export const updateMonthlyPurchase = createAsyncThunk('monthlyPurchase/updateMonthlyPurchase', async ({ id: monthlyPurchaseId, body }, thunkApi) => {
    const token = localStorage.getItem('im-user')
    const auth = { headers: { Authorization: `Bearer ${token}` } }
    console.log(monthlyPurchaseId)
    try {
        const response = await api_put(`${HOST_MONTHLY_PURCHASE}/monthlyPurchase/${monthlyPurchaseId}`, body, auth)
        console.log(response)
